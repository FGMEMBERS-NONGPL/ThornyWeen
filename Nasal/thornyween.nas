...
var loop = func {
	...
	if ((running.getValue() == 1) and (gear.getValue() > 0)) { 
	 throttle.setValue(1.25*elevator.getValue()+0.1-rpm.getValue()/2100);
	...
	if ((cranking.getValue() == 1) and ((gear.getValue() == 0) or (clutch.getValue == 1)) and (fuel.getValue()>=0.00000169)) {
	 running.setValue(1);	 
	 cranking.setValue(0);
	}
	...
	if (running.getValue() == 1) {
	 if (fuel.getValue() < 0.00000169) {
	  running.setValue(0);
	  }
	 else {
	  fuel_lev = fuel.getValue();
	  fuel.setValue(fuel_lev - (0.9*elevator.getValue()+0.1)*0.00000169);
	 }
	}
	...
	if (running.getValue() == 1) {
	 if (gear.getValue() == -1) {
	  ...
	  rev_throttle.setValue(0.9*elevator.getValue()+0.1-rpm.getValue()/2100);
	...
	mbrake = rpm.getValue()/2100-(1.0*elevator.getValue()+0.25*running.getValue());
	...
	if (gear.getValue() == 3) {
	 ...
	 setprop("/controls/gearstick-y", -8);
      ...
      if (gear.getValue() == 5) {
	 setprop("/controls/gearstick-x",  8);
	 setprop("/controls/gearstick-y", 8);
	}
	if (gear.getValue() == -1) {
	 setprop("/controls/gearstick-x", -8);
	 setprop("/controls/gearstick-y", 10);
	}
	if (gear.getValue() == 0) {
